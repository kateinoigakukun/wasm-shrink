...
use wasmtime_wast::link_spectest;
use wast::Wat;
use wast::{
...
fn run_wast(wast: &str) -> anyhow::Result<()> {
    ...
    let pass = WasmShrink::ConstParam;
    let mut wast_context = WastContext::new(store, pass);
...
// NOTE: The most of following parts are borrowed from https://github.com/bytecodealliance/wasmtime/blob/v0.30.0/crates/wast/src/wast.rs
...
impl<T, P: Pass> WastContext<T, P> {
    /// Construct a new instance of `WastContext`.
    pub fn new(store: Store<T>, pass: P) -> Self {
    ...
    fn module(&mut self, instance_name: Option<&str>, module: &[u8]) -> Result<()> {
        let module = &self.pass.run(module).context("optimization failed")?;
